...
``src/orc/conf/local.py``. You can base this file on the
...
1. Clone or download the code from `Github`_ in a folder like
   ...
   .. code-block:: bash
       ...
       $ git clone git@github.com:maykinmedia/gemma-mock-overigeregistratiecomponenten.git orc
       Cloning into 'orc'...
       ...
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting orc_db_1 ... done
       Starting orc_web_1 ... done
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it orc_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it orc_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/orc/
...
environment variables. See ``src/orc/conf/docker.py`` for
all settings.
...
.. code-block:: bash
    ...
    $ docker build . && docker run \
        -p 8000:8000 \
        -e DJANGO_SETTINGS_MODULE=orc.conf.docker \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
        -e DATABASE_HOST=... \
        --name orc

    $ docker exec -it orc /app/src/manage.py createsuperuser
